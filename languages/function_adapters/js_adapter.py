...
class JavaScriptAdapter:
    ...
    def call_js_function(self, function_name, args, js_code=None):
        try:
            ...
            exec_result = subprocess.run(['node', str(wrapper_file)], capture_output=True, text=True)
    ...
    def create_js_function_signature(self, function_name, param_count=0):
        ...
        return FunctionSignature(name=function_name, language='javascript', implementation=function_name, return_type='any', parameters=params)
    ...
    def extract_js_function_signature(self, js_code, function_name):
        import re
        patterns = [f'function\\s+{function_name}\\s*\\((.*?)\\)', f'const\\s+{function_name}\\s*=\\s*\\((.*?)\\)', f'let\\s+{function_name}\\s*=\\s*\\((.*?)\\)', f'{function_name}\\s*:\\s*\\((.*?)\\)']
        for pattern in patterns:
            match = re.search(pattern, js_code)
            if match:
                params = match.group(1).split(',')
                params = [p.strip() for p in params if p.strip()]
                return {'name': function_name, 'parameters': params, 'parameter_count': len(params)}
        return {'name': function_name, 'parameters': [], 'parameter_count': 0}
